...
contract("BookStore", (accounts) => {
  describe("Publishing", async () => {
    it("gives the author the specified amount of book version copies", async () => {
      ...
      // const book_id = 1;
      const price = web3.utils.toWei("50", "ether"); // NOTE: Decimals incorrect for USDC
    ...
    it("increases the book ID", async () => {
      ...
      const price = web3.utils.toWei("50", "ether"); // NOTE: Decimals incorrect for USDC
      const currency = "0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48" // USDC Contract Address
    ...
    it("correctly sets the price and currency for a book versions", async () => {
      const store_front = await StoreFront.new();
      const book_store = await BookStore.new();
      ...
      // const book_id = 1;
      let price = web3.utils.toWei("50", "ether"); // NOTE: Decimals incorrect for USDC
      const currency = "0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48" // USDC Contract Address
      const quantity = 100;
      ...
      price = web3.utils.toWei("100", "ether"); // NOTE: Decimals incorrect for USDC
      await book_store.publish(quantity, price, currency, { from: author });
  ...
  describe("Purchasing from the author", async () => {
    it("transfers the purchase tokens to the author", async () => {
      const store_front = await StoreFront.new();
      const book_store = await BookStore.new();
      ...
      // const book_id = 1;
      let price = web3.utils.toWei("5000", "ether"); // NOTE: Decimals incorrect for USDC
  ...
  describe("Security", async () => {
    it("won't allow anyone but the store front to call BookStore#transferFromAuthor", async () => {
      ...
      await truffleAssert.reverts(book_store.transferFromAuthor(accounts[3], 1, { from: accounts[3] }), "Method can only e called by Store From contract");
    });
